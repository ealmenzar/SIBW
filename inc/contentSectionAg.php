...
if (isset($_GET["edit"])) {
    ...
    if(isset($_POST["nombre"],$_POST["relacion"])) {
        ...
        $query = "UPDATE etiquetas SET nombre = '$nombre',relacion = '$relacion' WHERE id ='$id'";
        $link->query($query);
    ...
    if(isset($_GET["add"])){
        if (isset($_POST["nombre"], $_POST["relacion"])) {
            ...
            if($_POST["relacion"]==1){
                echo " <div class='edit'>
                        <form method='post' action=''
                           <h3>Nombre: </h3><input type='text' name='nombre' value='".$_POST["nombre"]."'><br>
                           <h3>Asociado a la sección: </h3>
                           <select name='seccion'>";
                            $sub=getAllTags($link);
                            ...
                            echo "</select><br><input type='submit' value='Guardar'><br>
                        </form>
                       </div>";
            } else {
                $query = "INSERT INTO etiquetas (id, nombre, relacion) VALUES (null, '$nombre', '$relacion')";
                $link->query($query);
                header("location:index.php?tpl=SectionAg");
            }
        } else {
            if(isset($_POST["seccion"], $_POST["nombre"])){
                $seccion = str_replace("'", "\'", $_POST["seccion"]);
                $nombre = str_replace("'", "\'", $_POST["nombre"]);
                $query = "INSERT INTO etiquetas (id, nombre, relacion) VALUES (null, '$nombre', '$seccion')";
                $link->query($query);
                header("location:index.php?tpl=SectionAg");
            } else {
                echo "<div class='edit'>
                    <form method='post' action=''>
                        <h3>Nombre</h3><input type='text' name='nombre' required><br>
                        <h3>Relación</h3><select name='relacion' required>
        ...
        $arraySections=getSectionsToAg($link);
        ?>
        <a class="add-btn" href="index.php?tpl=SectionAg&add=1">Añadir nueva sección o subsección</a>
        <table class="white-table">
            ...
            foreach ($arraySections as $section) {
                echo "<tr>
                        ...
                        <td>";if($section['relacion'] == 0) echo'Sección'; else echo'Subsección';
                echo "</td>
                        <td>".'<a style="display:inline-block" href="index.php?tpl=SectionAg&remove='.$section['id'].'">
                            ...
                            <a style="display:inline-block" href="index.php?tpl=SectionAg&edit='.$section['id'].'">
